...
exp_range = [0.7,0.8,0.75,0.15,0.25]
...
# opt_metric = 'loss_val'
run_id = 'lamda_gama_fix_gama_resnet02'
gama = 0.1
...
for opt_metric in ["val_f1_score",'val_loss','val_roc_auc_score']:

    # for lamda,gama in exp_range_2:
    for lamda in exp_range:
        bashCommand = f"python train_matan_model.py --research_type csce_2 --n_epochs 20 --n_procs 1 --batch_size 32 --device_id 0 --optim SGD --data_path ../datasets/DermMel/ --model_architecture resnet50 --lamda {lamda} --gama {gama} --is_mock 0 --momentum 0.9 --patience 5 --min_delta 1.0 --sch_gamma 1.0 --sch_step_size 5 --weight_decay 5.0e-2 --lr 1.0e-3 --opt_metric {opt_metric} --run_id {run_id}"
        print(bashCommand)
        process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
        output, error = process.communicate()
        
        
    bashCommand = f"python train_matan_model.py --research_type wce --n_epochs 20 --n_procs 1 --batch_size 32 --device_id 0 --optim SGD --data_path ../datasets/DermMel/ --model_architecture resnet50 --lamda {count_mainor / count_majorty} --is_mock 0 --momentum 0.9 --patience 5 --min_delta 1.0 --sch_gamma 1.0 --sch_step_size 5 --weight_decay 5.0e-2 --lr 1.0e-3 --opt_metric {opt_metric} --run_id {run_id}"
    print(bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()

    bashCommand = f"python train_matan_model.py --research_type ce --n_epochs 20 --n_procs 1 --batch_size 32 --device_id 0 --optim SGD --data_path ../datasets/DermMel/ --model_architecture resnet50 --lamda {lamda} --is_mock 0 --momentum 0.9 --patience 5 --min_delta 1.0 --sch_gamma 1.0 --sch_step_size 5 --weight_decay 5.0e-2 --lr 1.0e-3 --opt_metric {opt_metric} --run_id {run_id}"
    print(bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
